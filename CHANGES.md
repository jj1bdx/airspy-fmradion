...
* dev 20260924: Moved `main.cpp`'s per-block buffers (`if_shifted_samples`, `if_samples`, `audiosamples`) out of the main loop so they keep their allocations between blocks, and made the FM (mono), AM, and NBFM decoders swap their output buffer into `audio` instead of moving it, so neither side has to reallocate. Decoded output is unchanged. This cuts heap allocation calls by 16.6% in FM mode and 33.3% in AM mode; CPU time and peak memory show no measurable change. Verified bit-exact against the previous build for FM (default, `-E 36`, `-X`, `-f narrow`), AM, and NBFM decodes, with no new compiler warnings. Reference: doc/MAIN_CPP_SWAP_20260924.md.
* dev 20260923: C++ object cleanup to keep the number of mutable objects small; decoded output is unchanged. Added `const` to locals, member variables, and member functions (including `Source`'s getters and all device overrides), passed strings and filter coefficients by `const` reference, removed a no-op `std::move` into `AudioOutput::write()`, and made `AmDecoder::process()` take its input by `const` reference, which removes one copy of every AM-mode IF block. `FmDecoder::process()` now swaps rather than moves the AGC output buffer, so the buffer keeps its allocation between blocks. Verified bit-exact against the previous build for FM (default, `-E 36`, `-X`, `-f narrow`), AM, and NBFM decodes, with no new compiler warnings. Reference: doc/CPP_OBJECTS_20260923.md.
* dev 20260813: Re-evaluated the multipath filter's 20-block startup wait through the Quadrature Multipath Monitor output (`-X`), where the ideal output is known to be zero, so the absolute residual level is a quality figure rather than a difference from a baseline decode. **No code change.** A 20-block wait gives a lower whole-file quadrature residual than a 50-block wait in all 21 (recording x stage-count) cells, by 0.08 to 13.80 dB. The mechanism is the pilot-lock mute window: QMM output is digital zero until the PLL declares lock at a fixed 0.2 s, and a 20-block wait engages the filter at 0.107 s, inside that window, so the engagement transient decays before it can be emitted; a 50-block wait engages at 0.267 s and emits a burst 19 to 40 dB above the noise floor. The worst wait is 37 blocks, which lands engagement on the lock instant itself. The effect is confined to the first second, and it is device-dependent, since the wait is counted in blocks while the mute window is fixed in time. Reference: doc/MULTIPATH_QMM_20260813.md.
* dev 20260812: Changed `FmDecoder::m_wait_multipath_blocks`, the number of IF blocks the multipath filter is bypassed for at startup, from 100 to 20. The wait exists because engaging the filter before the IF AGC has ramped up is catastrophic: at `-E 100` with no wait at all, the decoded audio is uncorrelated with the correct output on some recordings and the pilot PLL rails against its +-30 Hz limiter. A 20-block wait removes all of that (every metric sits at the 100-block baseline across all seven `test-files/` recordings at `-E 18`, `-E 36`, and `-E 100`) while engaging the filter 4.3x sooner, inside the PLL's own acquisition, where it also costs less settling jitter. The onset of the failure is between 5 and 10 blocks, so 20 keeps a 4x margin. Reference: doc/MULTIPATH_WAITING_20260812.md.
* dev 20260725: Added tools to measure the sample-clock frequency offset of macOS audio interfaces, for both the DAC (output) and the ADC (input) side, at 44.1, 48 and 96 kHz. No airspy-fmradion code is affected; these are measurement tools for calibrating the interfaces the latency work depends on. Reference: doc/OFFSET_MEASUREMENT_20260725.md.
  * doc/clock_offset_measure.py drives the sweep and is the tool to run (`--list-devices` lists PortAudio indices, the same numbering as `-P`); doc/clock_offset_analyze.py holds the estimators and re-analyzes saved captures with no hardware present; doc/coreaudio_rate.py enumerates CoreAudio devices and gets/sets their nominal sample rate.
  * Three estimators per rate: the DAC and the ADC rate against the host clock from PortAudio's per-callback timestamps, plus the DAC/ADC ratio from the recorded tone's phase slope, which never reads the host clock. Three measurements of two unknowns leaves a spare degree of freedom for a consistency check.
  * **PortAudio and CoreAudio resample silently.** PortAudio opens a stream at any rate a device claims to accept, and macOS inserts an `AudioConverter` when that is not the device's nominal rate, so the fitted offset then describes the resampler rather than the clock. Neither `check_input_settings()` nor the opened stream's `samplerate` attribute detects it: both report 88200 Hz on a Roland Rubix24 that has no 88.2 kHz mode at all. The tools read and set the nominal rate through CoreAudio directly, and restore it afterwards. The difference is worth 10 ppm (-192.3 ppm resampled against -179.1 ppm switched).
  * Measured on a Mac mini 2023: the FiiO K7 DAC is -7.2 ppm at 44.1 kHz and about -12.1 ppm at 48 and 96 kHz; the Rubix24 ADC is -190.3 ppm at 44.1 kHz and about -179.0 ppm at 48 and 96 kHz. **Both interfaces are rate-dependent, so a calibration taken at one rate does not transfer to another.** Runs reproduce to 0.11 ppm, and thermal drift moves every figure coherently by a few tenths of a ppm over an hour.
  * This corrects the -7.33 ppm recorded for the K7's 48 kHz DAC in doc/LATENCY_MEASUREMENT_20260725.md section 3, which was taken with the K7 left at 44.1 kHz nominal and therefore resampled; the true figure is about -12.1 ppm. That document's Rubix24 calibration (-189.54 ppm) was unaffected, because 44.1 kHz was that device's nominal rate, and reproduces here to under 1 ppm.
  * doc/loopback_clock_cal.py now takes PortAudio device indices instead of device names, gains `--list-devices`, validates the device pair, and warns when a device is not at the rate it is about to request. It keeps its marker chirps and the analog path delay they measure, which the new tools drop to keep the tone clean. With the rates actually set that delay is 1.095 ms rather than the 1.316 ms recorded earlier, the difference being the `AudioConverter`'s own latency.
* 20260725-0: Major refactoring, plus the first end-to-end latency measurements. Three changes alter on-air behavior: the multipath filter adaptation rate above `-E 36`, the pilot PLL retune, and the shorter stereo lock time. The changes include:
...
* dev-lpf-volk 20260725: Rewrote the FIR low-pass filters `LowPassFilterFirIQ` and `LowPassFilterFirAudio` (`sfmbase/Filter.cpp`) to compute each output as a single VOLK dot product. Decoded audio matches the previous code to ~134 dB signal-to-difference (the float32 re-association floor); whole-pipeline user CPU dropped about 8 percent. Reference: doc/LPF_VOLK_20260725.md.
  ...
  * Fixed a latent warm-up bug: the old warm-up loops summed taps `j = 1 .. order` and dropped the `coeff[0]` tap on the first `order` outputs of every block. It was nearly harmless because every shipped coefficient table is a tapered windowed sinc with an edge tap <= -67 dB (exactly 0 for the 3-tap IF filter of the default and wide FM presets).
* dev-multipath-exp 20260724: Reviewed, measured, and revised the FM multipath filter (`MultipathFilter`, CMA/Godard blind equalizer), and added the first ground-truth harness for it (doc/make_two_ray_channel.py synthesizes a static or fading two-ray channel, doc/eval_two_ray_snr.py scores a decode against the clean decode). Reference: doc/MULTIPATH_FILTER_DESIGN_20260724.md (section 0 lists the diff, section 17 the ground-truth results).
  * Removed six of the eleven O(N) passes per four input samples: the delay line is now a ring buffer holding each sample twice, so the newest N samples stay contiguous for VOLK without the per-sample `m_state.erase(m_state.begin())` shift, and the NLMS power sum is maintained incrementally instead of being recomputed into a heap-allocated scratch buffer on every update (exactness restored every 65536 updates). Measured 1.6x faster for `-E36` through `-E200`, with decoded audio matching to -104 dB.
  * Scaled the NLMS `alpha` with the filter order, clamped at 0.5, so the per-tap adaptation rate `mu = alpha / sum(|state|^2)` stays at its `-E36` value instead of falling as 1/N when `-E` is raised. `-E36` is unchanged by construction; worth +6.1 dB of post-discriminator SNR at `-E100`. **This is the only change that alters on-air behavior.**
  * Hardened divergence handling and cleaned up the interface: `FmDecoder` now clears the delay line as well as the coefficients on a divergence reset (the offending non-finite sample used to stay in the delay line and cause a burst of resets), and `process()` bounds the output and error magnitude at 10 instead of only rejecting non-finite values, written as "not within the limit" so NaN still trips it. Removed the unreachable `get_reference_level()`, made the coefficient/error accessors `const`, and added `-E` sizing advice to the help text: **the stage count must be sized to the echo delay spread; more stages is not safer** (`-E200` measures 6.9 dB *worse* than the filter off on a 3 microsecond echo), and the filter is a net loss on shallow or fading channels, which is the mechanism behind the long-standing "turn off if reception becomes unstable" caution.
* dev-pll-fix 20260723: Shortened the `PilotPhaseLock` lock-decision time from 0.5 second to 0.2 second, and changed its private counter members from `int` to `unsigned int` for clarity.
* dev-std-atan2-pilot 20260723: Replaced the `fast_atan2f()` float table-lookup phase detector in `PilotPhaseLock` with the standard double `std::atan2()`. Both phasor inputs are already double, so this drops a redundant narrowing and yields a correctly-rounded phase error; per-call speed is platform dependent (faster on arm64/macOS, ~2x slower on x86_64/glibc) but a negligible fraction of one core, and the decoded output is unchanged. References: doc/CORE_MATH_ATAN2F_20260722.md, doc/STD_ATAN2_X86_64_20260722.md. (This branch also carries the dev-pll-zeta-redesign 20260723 retune below.)
  * Removed the now-unused `fast_atan2f()` (GNU Radio 257-entry atan table plus linear interpolation, ~152 lines) from `include/Utility.h`; include swap `Utility.h` -> `<algorithm>`/`<cmath>`.
  * Removed the two unused second-stage phasor biquads `m_biquad_phasor_i2`/`_q2` from the `PilotPhaseLock` header.
* dev-pll-zeta-redesign 20260723: Retuned the FM stereo pilot PLL (`PilotPhaseLock`) so its dominant closed-loop pole pair is damped at zeta = 0.71 (was ~0.57, mildly under-damped), inside the conventional 0.70-0.73 band, with the loop natural frequency (fn = 22.3 Hz) and the loop structure unchanged. References: doc/PLL_REDESIGN_20260723.md and the analyses doc/PLL_ANALYSIS_20260722.md, doc/PLL_ANALYSIS_2_20260723.md, doc/PLL_ANALYSIS_3_20260724.md; two further experiments were measured and reverted, see doc/PLL_EXPERIMENT_20260724.md and doc/PLL_EXPERIMENT_2_20260724.md.
  * Widened the in-loop phasor biquad LPF (all-pole, real corners ~34/160 -> ~40/188 Hz).
  * Rescaled the FIR PI gains by x0.889 to hold fn constant.
  * Verified against an exact 5-state linear loop model (zeta = 0.7101, fn = 22.34 Hz, stable) and on test-files/piano_iqtest.wav: steady-state tracking, jitter, and pilot level unchanged (19000.012 Hz, std 0.045 Hz), acquisition first swing shrinks from -2.66 to -1.52 Hz, decoded audio identical to -90 dBFS.
* 20260716-0:
