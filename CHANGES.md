...
* dev-portaudio-latency-option 20260715: Added the new `-L`/`--portaudio-latency` option to set the PortAudio output suggested latency in milliseconds (valid range: 1 to 40, default: platform-dependent). When given, the value is passed verbatim to PortAudio as `suggestedLatency`, bypassing the platform minimum-latency floor. `-L` is ignored unless PortAudio output (`-P`) is used.
